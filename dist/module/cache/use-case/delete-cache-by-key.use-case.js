...
let DeleteCacheByKeyUseCase = class DeleteCacheByKeyUseCase extends use_case_base_1.BaseUseCase {
    ...
    async execute({ data }) {
        try {
            const { key, selected_db, server } = data;
            const keyIsExist = await this.redisCache.getCacheByKeys(key, selected_db, server);
            if (!keyIsExist)
                throw new common_1.UnprocessableEntityException("Key Doesn't Exists");
            await this.redisCache.deleteCacheByKeys(key, selected_db, server);
