...
let GetCacheByKeyUseCase = class GetCacheByKeyUseCase extends use_case_base_1.BaseUseCase {
    ...
    async execute({ data }) {
        try {
            const selectedDb = Number(data.selected_db);
            const value = await this.redisCache.getCacheByKeys(data.keys, selectedDb, data.server);
            const keyInfo = await this.redisCache.keyInfo(data.keys, selectedDb, data.server);
