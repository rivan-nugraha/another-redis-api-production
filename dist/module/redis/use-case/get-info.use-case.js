...
let GetInfo = class GetInfo extends use_case_base_1.BaseUseCase {
    ...
    async execute() {
        return this.executeWithServer();
    }
    async executeWithServer(server) {
        try {
            const selectedServer = server || this.redisInfoService.getDefaultServerKey();
            const info = await this.redisInfoService.serverInfo(selectedServer);
            const memory = await this.redisInfoService.memoryInfo(selectedServer);
            const stats = await this.redisInfoService.statsInfo(selectedServer);
            const key_statistics = await this.redisInfoService.getListDbKeys(selectedServer);
            const all_redis_info = await this.redisInfoService.getRedisInfo(selectedServer);
            return new response_dto_base_1.ResponseDto({
                status: common_1.HttpStatus.OK,
                message: 'Successfully retrieved Redis info',
                data: {
                    selected_server: selectedServer,
                    servers: this.redisInfoService.listServers(),
                    server: info,
                    memory: memory,
                    stats: stats,
                    key_statistics: {
