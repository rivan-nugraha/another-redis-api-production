...
let EnvService = class EnvService {
    constructor(configService) {
        ...
        this._redisServers = this.parseRedisServers();
    }
    get variables() {
        return {
            mode: this._mode,
            ...
            redisServers: this._redisServers,
    ...
    parseRedisServers() {
        const rawServers = this.configService.get(env_key_const_1.EnvKey.REDIS_SERVERS);
        if (rawServers) {
            try {
                ...
                const normalized = parsed
                    ...
                    .map((item, index) => ({
                    key: item.key || `redis-${index + 1}`,
                    label: item.label || item.key || `Redis ${index + 1}`,
                ...
                throw new Error(`Invalid REDIS_SERVERS configuration: ${error}`);
            }
        }
        if (!this._redisHost || !this._redisPort) {
            return [];
        }
        return [
            {
