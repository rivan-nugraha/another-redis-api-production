...
let RedisInfoService = class RedisInfoService {
    ...
    async serverInfo(serverKey) {
        const infoRaw = await this.redisRegistry.runWithClient((client) => client.info('server'), serverKey);
    ...
    async memoryInfo(serverKey) {
        const infoRaw = await this.redisRegistry.runWithClient((client) => client.info('memory'), serverKey);
        const infoLines = infoRaw.split('\r\n');
        const info = {};
        infoLines.forEach(line => {
            if (line && !line.startsWith('#')) {
    ...
    async statsInfo(serverKey) {
        const [statsRaw, clientsRaw] = await Promise.all([
            this.redisRegistry.runWithClient((client) => client.info('stats'), serverKey),
            this.redisRegistry.runWithClient((client) => client.info('clients'), serverKey),
        ]);
        const info = {};
    ...
    async getListDbKeys(serverKey) {
        const infoRaw = await this.redisRegistry.runWithClient((client) => client.info('keyspace'), serverKey);
        const infoLines = infoRaw.split('\r\n');
        const dbList = [];
        infoLines.forEach(line => {
    ...
    async getAllDBWithStats(serverKey) {
        return this.redisRegistry.runWithClient(async (client) => {
            ...
            const infoRaw = await client.info('keyspace');
    ...
    async getRedisInfo(serverKey) {
        const info = await this.redisRegistry.runWithClient((client) => client.info(), serverKey);
        return info
            .split('\n')
    ...
    listServers() {
        return this.redisRegistry.listServers();
    }
    getDefaultServerKey() {
        return this.redisRegistry.getDefaultServerKey();
    }
};
